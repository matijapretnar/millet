...
let implode cs =
  let buf = Buffer.create 79 in
  List.iter (Buffer.add_char buf) cs;
  Buffer.contents buf

let con n = 0x80 lor (n land 0x3f)

let rec encode ns = implode (List.map Char.chr (encode' ns))

and encode' = function
  | [] -> []
  | n :: _ when n < 0 -> raise Utf8
...
let rec decode s = decode' (List.map Char.code (explode s))
